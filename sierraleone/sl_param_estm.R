###### Sierra Leone: Model fitting, parameter estimation ------

# clear environment and set working directory
rm(list = ls())
wd = "~/sierra leone code"
setwd(wd)
...
sl.data = readRDS("sl_data_23Sept.rds")
data.f = sl.data[,c(2:4)]
...
# Note: starting date: "2014-06-02" = day 1
...
seirdb.sse = function(varparms, fixparms, times, start, data) {
  ...
  sse = sse.cases + 1.5*sse.deaths  #since death data is smoothed, weighted 1.5 times
...
for (i in 1:nsim) {
  ...
  sl.optim = optim(par = varparms, seirdb.sse, fixparms = fixparms, method = "L-BFGS-B",
                   ...
                   upper = c(log(2), log(2), log(1), log(300), logit(0.8), logit(0.8)))
...
saveRDS(all.estms, file = 'sl_final_24Sept.rds')
write.table(all.estms, file = "final_sl.csv")

model.estms = all.estms
#model.estms = readRDS('sl_final_24Aug.rds')
model.estms

# calculate r0 value
r0_sl = r0.fn2(model.estms)
r0_sl
...
pInc = ggplot(data.f, aes(x = Day, y = Cases)) +
  ...
  geom_line(data = model.f2, aes(x = time, y = Inc), col = "#00AFBB", size = 0.7) +
  ggtitle("Sierra Leone Cumulative Cases")
  #theme_minimal()
pInc
...
pB = ggplot(data.f, aes(x = Day, y = Deaths)) +
  geom_point(shape = 1, color="gray35") + 
  geom_line(data = model.f2, aes(x = time, y = B), col = "#00AFBB", size = 0.7) +
  ggtitle("Sierra Leone Cumulative Deaths (Class B)") 
  #theme_minimal()
pB
...
#incidence
model.f3 = model.f2
model.f3$inc_predict = c(NA, model.f3[2:nrow(model.f3),8] - model.f3[1:(nrow(model.f3)-1),8] )

pI = ggplot(sl.data, aes(x = Day, y = inc)) +
  geom_point(shape = 1, color="gray35")  + ylab("Cases") +
  geom_line(data = model.f3, aes(x = time, y = inc_predict), col = "#00AFBB", size = 0.7) +
  ggtitle("Sierra Leone Case Incidence (Class I)")
  #theme_minimal()
pI
