###### Sensitivity analysis for Sierra Leone ------
...
## betaI
betaI_range = seq(0.05, 0.16, length.out = 500)
...
r0_betaI = ggplot(betaI_info, aes(x = betaI, y = r0)) +
  geom_line(size = 1.2, colour = "#00AFBB") + xlab(expression(beta[I])) + ylab(expression('R'[0])) +
  theme_minimal() +
...
## betaD
betaD_range = seq(0.5, 0.8, length.out = 500)
model.estms2 = model.estms
betaD_r0_range = NULL
...
r0_betaD = ggplot(betaD_info, aes(x = betaD, y = r0)) +
  geom_line(size = 1.2, colour = "#00AFBB") + xlab(expression(beta[D])) + ylab(expression('R'[0])) +
  theme_minimal() +
  theme( axis.title.x = element_text(size=rel(1.2)),
         axis.title.y = element_text(size=rel(1.2)))
#theme(axis.title.y = element_text(angle=0)) 
...
r0_lambda1 = ggplot(lambda1_info, aes(x = lambda1, y = r0)) +
  geom_line(size = 1.2, colour = "#00AFBB") + xlab(expression(lambda[1])) + ylab(expression('R'[0])) +
  theme_minimal() +
  theme( axis.title.x = element_text(size=rel(1.2)),
         axis.title.y = element_text(size=rel(1.2)))
#theme(axis.title.y = element_text(angle=0)) 
...
r0_lambda2 = ggplot(lambda2_info, aes(x = lambda2, y = r0)) +
  geom_line(size = 1.2, colour = "#00AFBB") + xlab(expression(lambda[2])) + ylab(expression('R'[0])) +
  theme_minimal() +
  theme( axis.title.x = element_text(size=rel(1.2)),
         axis.title.y = element_text(size=rel(1.2)))
#theme(axis.title.y = element_text(angle=0)) 
...
r0_mu = ggplot(mu_info, aes(x = mu, y = r0)) +
  geom_line(size = 1.2, colour = "#00AFBB") + xlab(expression(mu)) + ylab(expression('R'[0])) +
  theme_minimal() +
  theme( axis.title.x = element_text(size=rel(1.2)),
         axis.title.y = element_text(size=rel(1.2)))
#theme(axis.title.y = element_text(angle=0)) 
...
r0_rho = ggplot(rho_info, aes(x = rho, y = r0)) +
  geom_line(size = 1.2, colour = "#00AFBB") + xlab(expression(rho)) + ylab(expression('R'[0])) +
  theme_minimal() +
  theme( axis.title.x = element_text(size=rel(1.2)),
         axis.title.y = element_text(size=rel(1.2)))
#theme(axis.title.y = element_text(angle=0)) 
...
for ( i in 1:length(tc_range)){
  ...
  plot(fit[,1], fit[,8], type = "l", ylim = c(0,15000), xlim = c(0,800), 
       main = paste("tc =", tc_range[i]), xlab = "Days", ylab = "Cumulative Cases", col = "#00AFBB", lwd = 1.2)
...
tc_early = ggplot(df.cases_avoided, aes(x = days, y = cases, group = 1)) + 
  geom_line(size = 1.2, color = "#00AFBB") + xlab("Days sooner") + ylab("Cases avoided") +
...
p_thresh = ggplot(thresh_data, aes(x = threshold, y = duration)) + 
  geom_line(size = 1.2, color = "#00AFBB") + xlab("Threshold (number of cases)") + ylab("Duration of epidemic") +
  ggtitle( expression( paste("Sierra Leone (", eta, " = 0.619)"))) + 
...
#### Relationship between eta and duration of the epidemic keeping tc constant -----
eta_range2 = seq(0.25, 0.65, 0.01)
model.estms2 = model.estms
threshold_vals = c()
duration_vals = c()

for (i in 1:length(eta_range2)){
  model.estms2['eta'] = eta_range2[i]
  fit = ode(times = times, y = start, func = seirdb, parms = model.estms2)
  diff_inc = c(NA, fit[2:nrow(fit),8] - fit[1:(nrow(fit)-1),8] )
  #peak = which.max(diff_inc) #which(diff_inc <= 0)[1]
  slow_down = which(diff_inc < 1)[1]

  duration_vals = c(duration_vals, (slow_down)) #- model.estms2['tc']
}

thresh_data = data.frame(eta = eta_range2, duration = duration_vals)

p_eta = ggplot(thresh_data, aes(x = eta, y = duration)) + 
  geom_line(size = 1.2, color = "#00AFBB") + xlab(expression(eta)) + ylab("Duration of epidemic") +
  ggtitle( expression(paste("Sierra Leone (", t[C], " = 170)"))) + 
  scale_x_continuous(minor_breaks = seq(0.2, 0.65, 0.025), breaks = seq(0.2, 0.65, 0.05)) + 
  scale_y_continuous(minor_breaks = seq(0, 800, 50), breaks = seq(0, 800, 100)) +
  theme_minimal() +
  theme(text = element_text(size = 14))
p_eta

#### Assessing changes in eta -----

#4 plots - 3 monthly changes
eta_range = c(0.45, 0.5, 0.55, 0.6)
model.estms2 = model.estms
par(mfrow = c(2,2))

for ( i in 1:length(eta_range)){
  model.estms2['eta'] = eta_range[i]
  fit = ode(times = times, y = start, func = seirdb, parms = model.estms2)
  diff_inc = c(NA, fit[2:nrow(fit),8] - fit[1:(nrow(fit)-1),8] )
  #peak = which.max(diff_inc) #which(diff_inc <= 0)[1]
  slow_down = which(diff_inc < 1)[1]
  plot(fit[,1], fit[,8], type = "l", ylim = c(0,15000), xlim = c(0,800), 
       main = bquote(eta == .(eta_range[i])), 
                      xlab = "Days", ylab = "Cumulative Cases", col = "#00AFBB", lwd = 1.2)
       #main = paste("eta =", eta_range[i]), xlab = "Days", ylab = "Cumulative Cases", col = "#00AFBB", lwd = 1.2)
  abline(v = tc_range[i], lty = 2, col = "darkblue")
  abline(v = slow_down, lty = 2, col = "darkred")
  print(c(tc_range[i], slow_down, fit[800,8]))
}
...
samples = data.frame( betaI = qunif( lhsp[,1], 0.06, 0.18),   #check these two
                      betaD = qunif( lhsp[,2], 0.5, 0.9),     #check
...
ggplot(save.vals2, aes(values)) + 
  geom_histogram(binwidth = 0.2, fill = "#00AFBB", color = "grey45", alpha = 0.5) +
  xlab(expression("R"[0])) + ylab("Frequency") +
  ggtitle(expression(paste("Uncertainty analysis of ", "R"[0], " for Sierra Leone"))) +
  theme_minimal()
...
samples = data.frame( alpha = qnorm( lhsp[,1], 0.1, 0.0125),
                      ...
                      betaI = qunif( lhsp[,4], 0.05, 0.16),
                      betaD = qunif( lhsp[,5], 0.5, 0.8),
                      rho = qunif( lhsp[,6], 0.25, 1.5),
                      tc = qunif( lhsp[,7], 140, 200),
...
barplot(pcor_vals800, ylim = c(-1, 1), cex.names = 0.8, 
        ylab = "PRCC", main = "Sensitivity analysis of cumulative cases at day 800 for Sierra Leone",
        col = "#00AFBB")
...
sens_bar800 = ggplot(vals800, aes(x = variable, y = value)) +
  geom_bar(stat = "identity", fill = "#00AFBB", col = "grey45", alpha = 0.5) + 
  ylab("PRCC") + xlab("Parameter") + ggtitle("Sensitivity analysis of cumulative cases at day 800 for Sierra Leone") +
  theme_minimal()
